...
系统支持用户上传图片并获取 Markdown 格式的链接用于帖子或聊天中（例如: `![alt](https://.../uploads/1/pic.png`)。

使用方法:
- 进入设置 -> 修改个人资料页面，可以上传图片进行预览和复制 Markdown。
- 在发帖页面也可以上传图片并直接插入到编辑器。

安全与限制说明:
- 默认最大单张图片大小: 在 `config.py` 中可通过 `IMAGE_MAX_SIZE` 设置（默认为 5MB）。
- 全局上传大小限制: 受 `MAX_CONTENT_LENGTH`（`config.py`）控制（默认为 16MB）。
- 支持的图片格式: 由 `ALLOWED_IMAGE_EXTENSIONS` 控制 (例如 png/jpg/jpeg/gif/webp)。
- 上传文件会存放在 `static/uploads/<user_id>/` 目录下。
- 后端会检查文件内容是否与图片格式匹配（使用 Pillow 或 imghdr），并且使用安全文件名。

注意: 如果你需要更严格的图片审查或生成缩略图，建议在服务器上添加进一步处理与扫描功能。
