    ...
    try {
        ...
        chatSocket.on('message', (data) => {
            ...
            if (data.client_id && pendingMessages.has(data.client_id)) {
                ...
                // 标记为已处理（使用服务器ID如果有，否则使用client_id短期占位）
                if (data.id) {
                    processedMessageIds.add(data.id);
                } else {
                    processedMessageIds.add(data.client_id);
                }
            } else {
                // 新消息
                addMessageToUI(data);
...
function updateExistingMessage(clientId, serverMessage) {
    ...
    // 如果当前用户有删除权限，为更新后的消息添加删除按钮（有可能最初是临时ID，未添加）
    try {
        const existingMessage = document.querySelector(`[data-message-id="${serverMessage.id}"]`);
        if (existingMessage) {
            const hasDel = existingMessage.querySelector('.message-delete-btn');
            if (!hasDel) {
                const userElement = existingMessage.querySelector('.message-user');
                // 仅当 userElement 存在且权限允许时添加按钮
                ...
                if (userElement && (canDeleteAll || canDeleteOwn)) {
                    ...
                    delBtn.addEventListener('click', function (e) {
                        e.stopPropagation();
                        deleteChatMessage(serverMessage.id, existingMessage);
                    });
